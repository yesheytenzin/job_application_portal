...
RSpec.describe 'Api::Shared::Profiles', type: :request do
  ...
  describe 'GET /api/shared/profiles' do
  ...
  describe 'PUT /api/v1/profiles' do
    context 'when auth with a profile' do
      ...
      context 'with valid params' do
        ...
        it 'updates the profile' do
          put api_shared_profile_path, params: valid_params
          expect(response).to have_http_status(:ok)
      ...
      context 'with invalid params' do
        ...
        it 'does not update the profile' do
          put api_shared_profile_path, params: invalid_params
    ...
    context 'when authenticated without a profile' do
      ...
      it 'returns create profile obj in memory and is updated on top' do
        put api_shared_profile_path, params: { profile: { first_name: 'X', last_name: 'Y', username: 'yt', phone: '17509727', address: 'th'  } }
        expect(response).to have_http_status(:ok)
    ...
    context 'when unauthenticated' do
      it 'returns 401' do
        put api_shared_profile_path, params: { profile: { first_name: 'X' } }
        expect(response).to have_http_status(:unauthorized)
      end
    end
  end
